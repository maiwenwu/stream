var debugUpdateMgr;
$(function() {
	if (debugUpdateMgr == undefined) {
		debugUpdateMgr = new DebugUpdateMgr();
	}
});

function DebugUpdateMgr() {
	var self = this;
	self.init();
}

DebugUpdateMgr.prototype = {
	constructor : DebugUpdateMgr,
	init : function() {
		var self = this;
...
//		self.initTableHeight();
	...
	init_event : function() {
		...
		$('#check_all').click(function() {
			var ch=$("#module_list .tr-link input[type=checkbox]");
	...
	init_view : function() {
		...
		for (var i = 0; i < self.heart_info.length; i++) {
			list += "<tr id=\"module" + self.heart_info[i].board_id
					...
					+ "\" class=\"tr-link\">";
			list += "<td ><input type=\"checkbox\" value=\""+ i +"\" class=\"check_line\"/></td>";
			...
			list += "<td class=\"update_status" + self.heart_info[i].board_id + self.heart_info[i].module_id + "\">Online</td>";
			list += "</tr>";
		}
		...
		$('.check_line').click(function(e){
			...
			var choicelength=$('.check_line' ).length;
		    var choiceselect=$(".check_line:checked").length;
	...
	checkOnline : function() {
		...
		for (var i = 0; i < self.heart_info.length; i++) {
			if (self.heart_info[i].online_status == 1) {
				$('#module' + self.heart_info[i].board_id + self.heart_info[i].module_id + ' input[type=checkbox]').removeClass("check_line");
				$('#module' + self.heart_info[i].board_id + self.heart_info[i].module_id + ' input[type=checkbox]').attr("disabled",true);
				$('#module' + self.heart_info[i].board_id + self.heart_info[i].module_id).removeClass("tr-link");
				$('#module' + self.heart_info[i].board_id + self.heart_info[i].module_id).removeClass("tr-in");
				$('#module' + self.heart_info[i].board_id + self.heart_info[i].module_id).addClass("Offline");
				$('.update_status' + self.heart_info[i].board_id + self.heart_info[i].module_id).html("Offline");
			} else {
				$('.stream' + self.heart_info[i].board_id + self.heart_info[i].module_id).addClass("tr-link");
	...
	init_data : function() {
		...
		self.refresh_update_info = setInterval(() => {
			self.getHeartInfo();
		}, 5000);
		self.info = [
...
//				window.location.reload();
	...
	{
		var self = this;
		clearTimeout(self.refresh_update_info);
		self.getHeartInfoIntervals = setInterval(function() {
			...
			for (var i = 0; i < self.checkArray.length; i++) {
				...
				if (self.heart_info[self.checkArray[i]].update_status == 0) {
					$('#module' + self.heart_info[self.checkArray[i]].board_id + self.heart_info[self.checkArray[i]].module_id + " .update_status"  + self.heart_info[self.checkArray[i]].board_id + self.heart_info[self.checkArray[i]].module_id).html("Update: successful" );
					...
					$('#module' + self.heart_info[self.checkArray[i]].board_id + self.heart_info[self.checkArray[i]].module_id + " .update_status"  + self.heart_info[self.checkArray[i]].board_id + self.heart_info[self.checkArray[i]].module_id).html("Update: " + self.heart_info[self.checkArray[i]].update_percent + "%");
				} else if (self.heart_info[self.checkArray[i]].update_status == 2) {
					self.info[self.checkArray[i]].status = 1;
					$('#module' + self.heart_info[self.checkArray[i]].board_id + self.heart_info[self.checkArray[i]].module_id + " .update_status"  + self.heart_info[self.checkArray[i]].board_id + self.heart_info[self.checkArray[i]].module_id).html(self.heart_info[self.checkArray[i]].update_msg).css('color',"red");
				}
			}
	...
	getPCState : function() {
		...
		$.ajax({
			...
			success : function(data) {
				...
				console.log("pc_state",self.pc_state);
